...
# In this example, we'd like to showchase how to use the [**direction API**](https://openrouteservice.org/documentation/#/reference/directions/directions/directions-service) and to avoid a number of construction sites while routing.
#
...
# We take the [open data](https://www.opendata-hro.de/de/dataset/baustellen) from the Rostock authorities. It's hard (to impossible) to find construction site polygons, so these are points and we need to buffer them to a polygon to be able to avoid them when they cross a street. 
#
...
# Hui, that's a lot construction sites in Rostock! If you dig into the `properties` of the JSON, you'll see that those are kept up-to-date though. Seems like an annoying place to ride a car...
#
# Aaanyways, as you might know, a GET request can only contain so many characters. Unfortunately, > 80 polygons are more than a GET can take (HINT: that's why we set `resolution = 2`). And because there's no POST endpoint available currently, we'll have to work around it:
#
# One sensible thing one could do, is to eliminate construction zones which are not in the immediate surrounding of the route of interest. Hence, we can request a route without construction sites, take a reasonable buffer, filter construction sites within the buffer and try again. Let's try this:
